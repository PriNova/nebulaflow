...
import type { EdgeDTO, ExtensionToWorkflow, WorkflowToExtension } from '../../services/Protocol'
...
export const useWorkflowExecution = (
    ...
    const [nodeAssistantContent, setNodeAssistantContent] = useState<Map<string, AssistantContentItem[]>>(
        new Map()
    )
    ...
    const onExecute = useCallback(() => {
        ...
        vscodeAPI.postMessage({
            ...
            data: {
                nodes: updatedNodes,
                edges: edges as unknown as EdgeDTO[],
                resume:
        ...
        (fromNodeId: string, seedsOutputs: Record<string, string>) => {
            ...
            for (const n of nodes) {
                if (n.type === NodeType.VARIABLE) {
                    ...
                    if (varName) {
                        const v = seedsOutputs[n.id] ?? undefined
    ...
    const onAbort = useCallback(() => {
        ...
        setIsExecuting(false)
        setIsPaused(false)
        vscodeAPI.postMessage({ type: 'abort_workflow' })
    }, [abortController, vscodeAPI])

    const onPauseToggle = useCallback(() => {
        if (!isPaused) {
            // Request pause
            vscodeAPI.postMessage({ type: 'pause_workflow' })
            setIsPaused(true)
        } else {
